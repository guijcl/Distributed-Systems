...
/* Esta função liga so ao backup server
 */
int network_connecttobck(struct rtree_t *rtree);

/* Esta função envia para o backup
 */
struct message_t *network_send_backup(struct rtree_t * rtree,
                                       struct message_t *msg);

/* Esta função gere se pode ou não haver escrita
 */
